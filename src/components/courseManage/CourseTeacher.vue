    ...
    <div>
        <div>
            <div style="position: relative; height: 60px;">
                <p class="pull-left title">教师</p>
                <div class="pull-right title-button">
                    <el-button type="primary" icon="el-icon-plus" @click="showModal=true" :disabled="!isTeacher">邀请教师</el-button>
                </div>
            ...
            <el-table-column label="操作" align="center">
                <template slot-scope="scope">
                    ...
                    <el-button size="mini" type="danger" plain :disabled="!isTeacher"
                            @click="handleDelete(scope.row.userId)">移除教师</el-button>
                </template>
            </el-table-column>
        </el-table>
    </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            loadData(){
                this.loading = true;
                let url = `${HOST}/course_manage/get_teacher/${getCourse().id}`;
            ...
            handleDelete(userId){
                const _this = this;
                this.$confirm('确定要移除教师？', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    if(userId === getCourse().teacherId){
                        _this.$message.error("无法移除课程创建者！");
                        return
                    }
                    ...
                    let url = `${HOST}/course_manage/remove_teacher`;
